"""
Prepare SQL Query Function for API.
"""
...
def select_project_by_name(db: Session, project_name: str) -> schemas.Project:
    """
    Select project by project_name.
    """
    return (
...
def select_model_by_id(db: Session, model_id: int) -> schemas.Model:
...
def select_experiment_by_model_id(
    ...
    return (
        db.query(models.Experiment).filter(models.Experiment.model_id == model_id).all()
    )
